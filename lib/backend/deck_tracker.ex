defmodule Hearthstone.DeckTracker do
  ...
  def deck_stats(criteria) do
    base_deck_stats_query(criteria)
  ...
  defp base_aggregated_deck_stats_query() do
    from(ds in DeckStats,
      ...
      select: %{
        ...
        total: sum(ds.total),
        winrate: sum(ds.wins) / sum(ds.total),
        deck_id: ds.deck_id
      }
    )
  end
  ...
  defp base_deck_stats_query(criteria) do
    case criteria |> Enum.to_list() |> List.keyfind("use_aggregated", 0, :nomatch) do
      {"use_aggregated", _} -> base_aggregated_deck_stats_query()
      _ -> base_deck_stats_query()
    end
  end
