defmodule Backend.PlayerCountryPreferenceBag do
  ...
  def handle_cast({:update_user, %{cross_out_country: coc, show_region: sr} = user}, %{table: table} = state) do
  ...
  defp update_user(%{battletag: battletag, cross_out_country: coc, show_region: sr, country_code: country}, table)
  ...
  def get(btag, country) when is_binary(btag) and is_binary(country),
    do: table() |> Util.ets_lookup(key(btag, country), default())
