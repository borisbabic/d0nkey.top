defmodule TwitchBot.ConfigManager do
  ...
  def config(chat) do
    map =
      case Application.fetch_env(:backend, :twitch_bot_message_config) do
        {:ok, c} -> c
        _ -> test_config_map()
      end
  ...
  def quoter(sender, sender_in_reply), do: new("#{sender_in_reply} \"{{ message }}\"", %{sender: sender})

  def deck(response \\ "{{ deck_url }}"), do: new(response, %{type: :deck})
end
