defmodule Components.Socials do
  ...
  def paypal(assigns) do
    ~H"""
      <a href={@link} aria-label="PayPal Donation">
  ...
  def discord(assigns) do
    ~H"""
      <a href={@link} class={@class} aria-label="Discord Community">
  ...
  def twitch(assigns) do
    ~H"""
      <a href={@link} aria-label="Twitch Channel">
  ...
  def twitch_channel(assigns) do
    ~H"""
      <a href={"https://www.twitch.tv/#{@channel}"} aria-label={"Twitch Channel #{@channel}"}>
        <img style={"height: #{@height}px;"} class="image" alt="Twitch" src="/images/brands/twitch_extruded_wordmark_purple.svg" />
  ...
  def patreon(assigns) do
    ~H"""
      <a href={@link} aria-label="Patreon">
        <.patreon_image />
      </a>
    """
  end
