defmodule Components.Card do
  ...
  def render(assigns) do
    ~F"""
      <a id={"card_component_#{@card.id}"} aria-label={@card.name} href={if @disable_link, do: "javascript:;", else: ~p"/card/#{@card}"} class={"tw-relative", "card-image-container", "has-no-pointer-events": @disable_link}>
