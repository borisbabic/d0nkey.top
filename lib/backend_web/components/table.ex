defmodule Components.Table do
  @moduledoc "Drop in replacement for Components.SurfaceBulma.Table with new looks"
  ...
  def render(assigns) do
    ...
    <div>
      <.table id={"table_#{@id}"}>
                    ...
                    <a :on-click="sorted_click" phx-value-value={col.label} href="#">
                      <span>
                        ...
                        <span class="icon is-small">
                          <i class={"fa-solid", "fa-caret-up": @sort_reverse, "fa-caret-down": !@sort_reverse}></i>
                        </span>
                      </span>
                    </a>
                  {/if}
        ...
        <.tbody>
          <.trb :for={{item, index} <- Enum.with_index(@sorted_data)}>
  ...
  def update(assigns, socket) do
    assigns =
      assigns
      |> Map.put(:updated?, assigns[:data] != socket.assigns[:data])

    socket = assign(socket, assigns)

    socket = socket |> assign(:sorted_data, sorted_data(socket.assigns))
...
defmodule Components.Table.Column do
  @moduledoc """
