defmodule BackendWeb.Live.DecksTest do
  ...
  def deck_fixtures(base_dto, num \\ 101) do
    1..num
    |> Enum.map(fn _ ->
      base_dto
  ...
  test "includes wild highlander priest and warlock", %{conn: conn} do
    {:ok, view, html} = live(conn, "/decks?format=1")
    assert html =~ @warlock_code
    assert html =~ @priest_code
  end

  test "Legend excludes priest includes warlock decks", %{conn: conn} do
    {:ok, view, html} = live(conn, "/decks?format=1&rank=legend")
    refute html =~ @priest_code
    assert html =~ @warlock_code
  end

  test "Archetype exludes warlcok and includes priest", %{conn: conn} do
    {:ok, view, html} = live(conn, "/decks?format=1&archetype=Highlander Priest")
